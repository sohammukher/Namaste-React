...
import {addGptFoodResult} from '../utils/gptSlice'
...
const GptSearchBar = () => {
    ...
    const searchFoodItem = async(foodItem) =>{
        ...
        // const data = await fetch("https://api.edamam.com/api/food-database/v2/parser?app_id=042f3ec0&app_key=58bfafaa8ec7edba05d3d30f8dd3a4f6&ingr="+foodItem+"&nutrition-type=logging&category=generic-foods&category=generic-meals");
        const data = await fetch("https://api.edamam.com/api/food-database/v2/parser?app_id=042f3ec0&app_key=58bfafaa8ec7edba05d3d30f8dd3a4f6&ingr="+foodItem+"&nutrition-type=logging&category=generic-foods");
    ...
    const handleGptSearchClick = async () =>{
        ...
        // Make GPT API Call to Get Food Results.
        ...
        const gptResults = await openai.chat.completions.create({
          ...
          let nonNullResults =  results.filter(result => result !== null && result !== undefined);

          nonNullResults = nonNullResults.filter(obj =>  obj.food?.hasOwnProperty('image'));


          console.log("Non Null FOOD RESULTS ",nonNullResults);


          dispatch(addGptFoodResult({foodNames:foodList,foodResults:nonNullResults}));
